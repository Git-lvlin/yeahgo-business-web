...
const TableList = () => {
  ...
  const selectAll = (e) => {
    const { target } = e;
    if (target.checked) {
      setSelectItem(data.filter(ele=>ele.subType!=3&&ele.subType!=4));
    } else {
      setSelectItem([])
    }
  }
  ...
  const getFieldValue = () => {
    ...
    return {
      ...
      orderTypes:orderTypes === 0 ?[2,3,4,11,17,18]:'',
      ...rest,
    }
  }
    ...
    <PageContainer>
          ...
          data.map(item => (
            <div className={styles.list} key={item.id}>
              ...
              <div className={styles.second}>
                <Space size="large">
                  {orderType === 2&& <Checkbox onChange={(e) => { select(e, item) }} disabled={item.subType==3||item.subType==4} checked={selectItem.find(it => it.id === item.id)} />}
              ...
              <div className={styles.body}>
                ...
                <div style={{ textAlign: 'center' }}>
                {
                  item.subType?<Tag style={{ borderRadius: 10,marginTop:'10px' }} color="#f59a23">{({ 4:'氢原子销售',3:'氢原子押金',2:'氢原子启动' }[item.subType])}订单</Tag>:
                  <Tag style={{ borderRadius: 10 }} color="#f59a23">{({ 2: '秒约', 3: '拼团', 4: '团约', 11: '1688', 17: '盲盒活动', 18: '签到活动' }[item.orderType] || '秒约')}订单</Tag>
                }
                </div>
                <div style={{ textAlign: 'center' }}>
                  {item.status === 2 && <><a onClick={() => { setDeliveryVisible(true); setOrderId(item.id);setSubType(item.subType) }}>发货</a> <br /></>}
      ...
      {deliveryVisible &&
        <Delivery
          ...
          subType={subType}
          contractUrl={data?.find(ele=>ele.id==orderId).contractUrl}
        />
      }
      {shipVisible &&
        <Ship
