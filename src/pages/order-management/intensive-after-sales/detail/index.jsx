...
import { refundOrderDetail, findReturnRecord, addressList } from '@/services/order-management/intensive-after-sales'
...
const Details = () => {
  ...
  useEffect(() => {
    findReturnRecord({refundId: params.id}).then(res=> {
  ...
  useEffect(() => {
    setLoading(true)
    refundOrderDetail({refundId: params.id}).then(res => {
